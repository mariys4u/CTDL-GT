...
# def foo(lst):  # Hàm foo có tham số là danh sách (list)
#     sum = 0  # Khởi tạo biến sum = 0
#     product = 1  # Khởi tạo biến product = 1

#     for i in lst:  # Duyệt qua danh sách (list)
#         sum += i  # sum= sum + i
#     for i in lst:  # Duyệt qua danh sách (list)
#         product *= i  # product = product * i

#     print(f"Sum = {sum}, Product = {product}")  # In ra kết quả tổng và sản phẩm của danh sách (list)


# # Gọi hàm foo với một danh sách (list)
...
# def printPairs(list):  # ham printPairs co tham so la list
#     for i in range(0, len(list)):  # duyet mang list
#         for j in range(i, len(list)):  # duyet mang list
#             print(f"{list[i]},{list[j]}")  # in ra cac cap phan tu cua mang list
...
# def reverse(list):
#     reverse_list = [list[len(list) - i - 1] for i in range(len(list))]
#     print(reverse_list)
...
# list1 = ([1, 2, 3, 4]) # khoi tao list1
# print(list1) # in ra list1 -> [1, 2, 3, 4]

# my_array1.insert(0, 6) # them phan tu 6 vao vi tri 0 cua mang my_array1
# list1.insert(0, 6) # them phan tu 6 vao vi tri 0 cua list1
